...
# Hyperparameters follow original papers
...
# # DLinear (original paper: seq_len=336, lr=0.005, batch=32, epochs=10)
...
# # PatchTST (original paper: seq_len=336, lr=0.0001, batch=128, epochs=100,
...
for model in Chronos Timer TimesFM; do
    for data in $DATASETS; do
        for pl in $PRED_LENS; do
...
# FUSION_MODELS="DLinear,PatchTST"
# for data in $DATASETS; do
#     for pl in $PRED_LENS; do
