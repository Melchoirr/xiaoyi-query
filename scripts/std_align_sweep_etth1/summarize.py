...
def config_id(row):
    return (
        f"sw{float(row['std_weight']):g}_"
        f"r{float(row['std_ratio_min']):g}_{float(row['std_ratio_max']):g}"
    )
...
def main(path):
    ...
    print("\nBest ETTh1 test rerank by pred_len")
    print("| pred_len | config | MSE | MAE | baseline MSE | delta MSE vs shape |")
    print("|---:|---|---:|---:|---:|---:|")
    ...
    print("\nAll ETTh1 test rerank rows")
    print("| pred_len | config | MSE | MAE | baseline MSE | delta MSE vs shape |")
    print("|---:|---|---:|---:|---:|---:|")
    for row in sorted(test_rerank, key=lambda item: (int(item["pred_len"]), config_id(item))):
        print(
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        raise SystemExit("usage: python summarize.py outputs/std_align_sweep_etth1/summary_all.csv")
    main(sys.argv[1])
